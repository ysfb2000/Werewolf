import { CampType, EventCodeType, RoleCodeType, SpecialRulesType, StepType } from '../../shared/enums';
...
export class StoryFlow {
  isProcessed = false;
  banPlayer: Player | undefined;

  constructor(private readonly story: Story) {}

  previous(): void {
    this.story.step -= 2;
    if (this.story.step < 1) {
      this.story.step = 1;
    }
  ...
  next(): void {
    if (this.story.isNight) {
      this.isProcessed = false;

      while (!this.isProcessed && this.story.step <= 100) {
        this.nightScene();
      }
    ...
    switch (this.story.step) {
      case 1:
        this.showJudgeMessage('天亮了, 请睁眼');

        if (this.story.isIncludeRole(RoleCodeType.Bear)) {
          this.showNotice('查看熊身边有没有狼，有则公布"熊咆哮了"，没有则公布"熊没有咆哮"，熊已出局的话公布没咆哮', 0, RoleCodeType.Bear);
          this.showNotice('', 0, RoleCodeType.Bear);
        }

        if (this.story.player.find((p) => p.role?.roleType === RoleCodeType.Elder && p.isDead === false)) {
          this.banPlayer = this.story.player.find((p) => p.isCanNotSpeak && p.isDead === false);
          if (this.banPlayer) {
        ...
        this.settlement();
        this.showNotice('');
        result = this.story.isEnd();

        if (result.isOK) {
          this.showJudgeMessage(`游戏结束, 胜利阵营是${this.story.winner}`, 0);
          this.showNotice(result.message, 0);
          this.showNotice('');
          this.story.saveToLocalStorage();
          return;
        }
        break;
  ...
  handleEvent(event: GameEvent): void {
    switch (event.eventType) {
      case EventCodeType.KnightDuelFail:
        this.story.player.forEach((player) => {
  ...
  getWitchPowerStatus(no: number): string {
    let message = '';
    const witch = this.story.player.find((p) => p.role?.roleType === RoleCodeType.Witch);

    if (no === 0) {
      message = witch?.hasPoison ? '毒药可使用' : '毒药不可使用';
    } else if (no === 1) {
      message = witch?.hasHeal ? '解药可使用' : '解药不可使用';
    }
  ...
  checkRoleIfDead(role: RoleCodeType): boolean {
    return !!this.story.player.find((p) => p.role?.roleType === role && p.isDead === true);
  }

  private resetDayState(): void {
    this.story.player
      .filter((p) => p.isDead !== true)
      .forEach((player) => {
        if (player.isGuarded) {
  ...
  private showJudgeMessage(message: string, showInStage = 0, role: RoleCodeType = RoleCodeType.UnKnown): void {
    if (role !== RoleCodeType.UnKnown && !this.story.isIncludeRole(role)) {
      return;
    }

    const judgeMessage = `<span class="judgeColor">法官:${message}</span>`;

    if (this.story.stages === showInStage) {
      this.story.history.push(judgeMessage);
      this.isProcessed = true;
    }

    if (showInStage === 2 && this.story.stages > 2) {
      this.story.history.push(judgeMessage);
      this.isProcessed = true;
    }
  ...
  private showNotice(notice: string, showInStage = 0, role: RoleCodeType = RoleCodeType.UnKnown): void {
    if (role !== RoleCodeType.UnKnown && !this.story.isIncludeRole(role)) {
      return;
    }

    if (this.story.stages === showInStage) {
      this.story.history.push(notice);
      return;
    }
  ...
  private nightScene(): void {
    switch (this.story.step) {
      ...
      case 2:
        if (this.story.stages === 1) {
          this.story.currentStepType = StepType.SelectMiracleMerchant;
        }
      ...
      case 3:
        this.story.currentStepType = StepType.Common;
      ...
      case 6:
        if (this.story.stages === 1) {
          this.story.currentStepType = StepType.SelectCupid;
        }
      ...
      case 10:
        this.showJudgeMessage('请情侣睁眼，互相确认', 1, RoleCodeType.Cupid);
      ...
      case 12:
        if (this.story.stages === 1) {
          this.story.currentStepType = StepType.SelectMechanicalWolf;
        }
      ...
      case 13:
        ...
        if (this.story.isIncludeRole(RoleCodeType.MechanicalWolf)) {
          const currentPlayer = this.story.player.find((p) => p.role?.roleType === RoleCodeType.MechanicalWolf && p.isDead === false);
          if (currentPlayer && !this.story.player.find((p) => p.camp === CampType.Werewolf && p.id !== currentPlayer.id && p.isDead === false)) {
      ...
      case 16:
        if (this.story.stages === 1) {
          this.story.currentStepType = StepType.SelectNightMare;
        }
      ...
      case 19:
        if (this.story.stages === 1) {
          this.story.currentStepType = StepType.SelectGuard;
        }
      ...
      case 22:
        if (this.story.stages === 1) {
          this.story.currentStepType = StepType.SelectEvilKnight;
        }
      ...
      case 24:
        ...
        if (this.story.stages === 1) {
          this.story.currentStepType = StepType.SelectWolf;
        }

        this.showJudgeMessage('请互相确认身份', 1);
      ...
      case 26:
        this.story.currentStepType = StepType.SelectPersonWhoKillByWolf;
      ...
      case 28:
        if (this.story.stages === 1) {
          this.story.currentStepType = StepType.SelectWolfBeauty;
        }
      ...
      case 31:
        ...
        if (this.story.stages === 1) {
          this.story.currentStepType = StepType.SelectShadowWolf;
        }
      ...
      case 35:
        if (this.story.stages === 1) {
          this.story.currentStepType = StepType.SelectWitch;
        }
      ...
      case 39:
        if (this.story.stages === 1) {
          this.story.currentStepType = StepType.SelectChanneler;
        }
      ...
      case 40:
        ...
        {
          const tempPlayer = this.story.player.find((p) => p.role?.roleType === RoleCodeType.Channeler && p.isDead === false);
          if (tempPlayer) {
            this.showNotice(tempPlayer.isFear ? '已被恐惧, 不能查验' : '查验，并告知被查验玩家的角色', 0, RoleCodeType.Channeler);
          }
        }
        this.showNotice('', 0, RoleCodeType.Channeler);
        break;
      ...
      case 42:
        if (this.story.stages === 1) {
          this.story.currentStepType = StepType.SelectSeer;
        }
      ...
      case 43:
        ...
        {
          const tempPlayer = this.story.player.find((p) => p.role?.roleType === RoleCodeType.Seer && p.isDead === false);
          if (tempPlayer) {
            this.showNotice(tempPlayer.isFear ? '已被恐惧, 不能查验' : '查验，并告知查验结果', 0, RoleCodeType.Seer);
          }
        }
        this.showNotice('', 0, RoleCodeType.Seer);
        break;
      ...
      case 45:
        if (this.story.stages === 1) {
          this.story.currentStepType = StepType.SelectDeamer;
        }
        this.showJudgeMessage('梦游者请睁眼', 0, RoleCodeType.Deamer);
        this.showNotice('记录梦游者的号码', 1, RoleCodeType.Deamer);

        if (this.checkRoleIfDead(RoleCodeType.Deamer)) {
          this.showNotice('梦游者已出局', 0, RoleCodeType.Deamer);
        }
      ...
      case 46:
        this.story.currentStepType = StepType.Common;
        this.showJudgeMessage('请梦游者选择梦游的人', 0, RoleCodeType.Deamer);
        this.showNotice('记录梦游的人', 0, RoleCodeType.Deamer);
        this.showNotice('', 0, RoleCodeType.Deamer);
        break;

      case 47:
        this.showJudgeMessage('梦游者请闭眼', 0, RoleCodeType.Deamer);
        this.showNotice('', 0, RoleCodeType.Deamer);
        break;

      case 48:
        if (this.story.stages === 1) {
          this.story.currentStepType = StepType.SelectDemonHunter;
        }
      ...
      case 51:
        if (this.story.stages === 1) {
          this.story.currentStepType = StepType.SelectElder;
        }
      ...
      case 54:
        if (this.story.stages === 1) {
          this.story.currentStepType = StepType.SelectBear;
        }
      ...
      case 56:
        if (this.story.stages === 1) {
          this.story.currentStepType = StepType.SelectFox;
        }
      ...
      case 59:
        if (this.story.stages === 1) {
          this.story.currentStepType = StepType.SelectPuffer;
        }
      ...
      case 61:
        if (this.story.stages === 1) {
          this.story.currentStepType = StepType.SelectWhiteCat;
        }
      ...
      case 63:
        if (this.story.stages === 1) {
          this.story.currentStepType = StepType.SelectHunter;
        }
      ...
      case 64:
        this.story.currentStepType = StepType.Common;
        this.showJudgeMessage('猎人的开枪状态是', 0, RoleCodeType.Hunter);
        this.showNotice(this.story.getHunterGunStatus() ? '可开枪' : '不可开枪', 0, RoleCodeType.Hunter);
        this.showNotice('', 0, RoleCodeType.Hunter);
        break;
      ...
      case 68:
        this.story.currentStepType = StepType.SelectKnight;
      ...
      case 70:
        this.story.currentStepType = StepType.SelectFoolman;
      ...
      case 72:
        this.story.currentStepType = StepType.SelectGargoyle;
      ...
      case 75:
        this.story.currentStepType = StepType.Common;
        if (this.story.edition.isSpecialRulesInclude(SpecialRulesType.Police)) {
          this.showJudgeMessage('大家请睁眼，现在是竞选警长阶段', 1);
          this.showJudgeMessage('要竞选警长的请举牌示意', 1);

          const playerNo = this.getRandomInt(1, this.story.player.length || 0);
          const speakerDirection = this.getRandomInt(0, 1) === 0 ? '左边' : '右边';
      ...
      case 76:
        if (this.story.edition.isSpecialRulesInclude(SpecialRulesType.Police)) {
          this.story.currentStepType = StepType.SelectPolice;
          this.showJudgeMessage('竞选警长的结果.....请大家闭眼投票', 1);
  ...
  private settlement(): void {
    const deadPersons: Player[] = [];

    this.story.player
      .filter((item) => item.isDead === false)
      .forEach((player) => {
        if (player.isKilledByWolf) {
          this.showNotice(`${player.id}号玩家被狼人刀了`, 0);
          player.isDead = true;

          if (this.story.player.find((p) => p.role?.camp === CampType.Werewolf && p.isDead === false && p.isConfused)) {
            this.showNotice('狼群被迷惑了, 狼刀失效', 0);
            player.isDead = false;
          }

          if (this.story.player.find((p) => p.role?.camp === CampType.Werewolf && p.isDead === false && p.isFear)) {
            this.showNotice('狼人被恐惧了, 狼刀失效', 0);
            player.isDead = false;
          }

          if (player.isGuarded) {
            if (this.story.player.find((p) => p.role?.roleType === RoleCodeType.Guard && p.isDead === false && p.isFear)) {
          ...
          if (player.isSimulationGuarded) {
            if (this.story.player.find((p) => p.role?.roleType === RoleCodeType.MechanicalWolf && p.isDead === false && p.isFear)) {
          ...
          if (player.isHealed) {
            if (this.story.player.find((p) => p.role?.roleType === RoleCodeType.Witch && p.isDead === false && p.isFear)) {
        ...
        if (player.isPoisoned) {
          ...
          if (player.role?.roleType === RoleCodeType.WolfKing) {
            this.showNotice(`${player.id}号玩家是狼王, 被毒出局不能发动狼王刀`, 0);
          }
        }
        ...
        if (player.isGiftPoisoned) {
          ...
          if (this.story.player.find((p) => p.isLucky && p.isDead === false && p.isFear)) {
        ...
        if (player.isKilledByDeamonHunter) {
          ...
          if (this.story.player.find((p) => p.role?.roleType === RoleCodeType.DeamonHunter && p.isDead === false && p.isFear)) {
        ...
        if (player.role?.roleType === RoleCodeType.EvilKnight) {
          if (player.isForetelled && player.hasDamageReflection) {
            const seer = this.story.player.find((p) => p.role?.roleType === RoleCodeType.Seer && p.isDead === false);
            if (seer) {
          ...
          if (player.isGiftForetelled && player.hasDamageReflection) {
            const lucky = this.story.player.find((p) => p.isLucky && p.isDead === false);
            if (lucky) {
          ...
          if (player.isPoisoned && player.hasDamageReflection) {
            const witch = this.story.player.find((p) => p.role?.roleType === RoleCodeType.Witch && p.isDead === false);
            if (witch) {
        ...
        if (player.isDreamed) {
          if (this.story.player.find((p) => p.role?.roleType === RoleCodeType.Deamer && p.isDead === false && p.isFear)) {
            this.showNotice(`梦游者被恐惧了, ${player.id}号玩家梦游状态失效`, 0);
          } else {
            ...
            if (player.role?.roleType === RoleCodeType.WolfKing) {
              this.showNotice(`${player.id}号玩家是狼王, 梦游两次出局不能发动狼王刀`, 0);
            }
          }
        }
        ...
        if (player.role?.roleType === RoleCodeType.WolfBeauty && player.isDead === true && !player.isFear) {
          const charmedPlayer = this.story.player.find((p) => p.isCharmed);
          this.showNotice(`${player.id}号玩家是狼美人, 魅惑了${charmedPlayer?.id}号玩家`, 0);

          if (this.story.player.find((p) => p.role?.camp === CampType.Werewolf && p.isDead === false && p.isConfused)) {
            this.showNotice('狼群被迷惑了, 魅惑失效', 0);
          } else if (charmedPlayer) {
            if (charmedPlayer.isDead === true) {
              this.showNotice(`${charmedPlayer.id}号玩家已经出局了, 所以没有人和狼美人一起殉情`, 0);
            } else {
        ...
        if (player.isGiveLuckyToWolf && player.isDead === false && !player.isFear) {
          this.showNotice(`${player.id}号玩家是奇迹商人, 给了狼人幸运, 出局`, 0);
          player.isDead = true;
        }

        if (player.isDead && !deadPersons.find((p) => p.id === player.id)) {
          deadPersons.push(player);
        }

        if (player.isDead && player.role?.roleType === RoleCodeType.Deamer && !player.isFear) {
          const dreamer = this.story.player.find((p) => p.isDreamed && p.isDead === false);
          if (dreamer) {
            this.showNotice(`${player.id}号玩家是梦游者, 梦游了${dreamer.id}号玩家, ${dreamer.id}号玩家一同出局`, 0);
            dreamer.isDead = true;
        ...
        if (player.isDead && player.isLover) {
          const lover = this.story.player.find((p) => p.isLover && p.id !== player.id);
          if (lover) {
      ...
      deadPersons.forEach((player) => {
        if (player.isDead) {
          ...
          if (player.role?.roleType === RoleCodeType.MechanicalWolf) {
            const condition = player.simulatedRole === RoleCodeType.Hunter
              && !player.isFear
              && !player.isPoisoned
              && !player.isGiftPoisoned
              && !player.isSimulationPoisoned
              && !player.isSilent
              && !player.isDreamed;

            this.showNotice(condition ? '机械猎人可开枪' : '机械狼处于中毒、梦游、恐惧和沉默状态下不可开枪', 0);
          }
        }
      });
